...
The [`era5()`](https://eliocamp.github.io/gadiera5/reference/era5.md)
function is the main entry point. It returns a data.table with
information of all available variables and their aggregation level, as
well as a description. This data.table can then be filtered and
subsetted as usual.
...
function sets the dates of dates of interest. The range can be specified
as full dates but setting year-month or just year is also supported.
...
to retrieve all the files. The return is a data.table with a list column
called `files`.
